...
1. **Project Part #1: Identify Files in a Top-Level Directory that will be Read into a DataFrame, based upon meeting certain criteria, and then uploaded to a Database**:
    ...
    E). Next, a DataFrame for each of the excel file paths included in the DataFrame table under bullet D)(I)(i)(2) will be created. The specific values to be pulled from these excel UW Models are included in an excel file within this project directory. As such, see below for the path to this file along with myriad other details :
