...
1. **Project Part #1: Identify Files in a Top-Level Directory that will be Read into a DataFrame, based upon meeting certain criteria, and then uploaded to a Database**:
    ...
    F). Next, the dataframe should be stored in a SQLite database. The database should be named underwriting_models.db and the table should be named underwriting_model_data. The table should have the same columns as the dataframe.
        (1) The database should be created in the same directory as the script. The code should also check if the database already exists and if it does, it should append the new data to the existing table. If the database does not exist, it should create a new database and a new table.
