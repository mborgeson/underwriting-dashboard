...
2. **Project Part #2: Create a Frontend Dashboard Using the Database Data from Part #1**:
    - A). The project will utilize the database created in Part #1 to create a dashboard that will allow the user to view the data in the underwriting_model_data table. The dashboard should be created using the agent's recommended dashboard package and should have the following features:
        ...
        (IV) A filter that allows the user to filter the data by file name, last modified date, sheet names, column names, property name, city, submarket, and unit count;
        (V) A download button that allows the user to download the data in the underwriting_model_data table as an xlsx file;
        (VI) Query functionality that allows the user to query the data in the underwriting_model_data table by file name, last modified date, sheet names, column names, property name, city, submarket, and unit count;
        (VII) A search bar that allows the user to search for specific data in the underwriting_model_data table; and
        (VIII) A button that allows the user to refresh the data in the underwriting_model_data table.
    A). This part of the project will entail creating a dashboard that will allow the user to view the data in the underwriting_model_data table. The dashboard should be created using the agent's recommended dashboard package and should have the following features:
        1) A title for the dashboard, 2) A description of the dashboard, 3) A table that shows all the data in the underwriting_model_data table, 4) A filter that allows the user to filter the data by file_name, last_modified_date, number_of_sheets, sheet_names, number_of_rows, number_of_columns, column_names, and data_types, 5) A download button that allows the user to download the data in the underwriting_model_data table as a CSV file.
    B).
